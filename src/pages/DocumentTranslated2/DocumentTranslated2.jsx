...
export default function DocumentTranslated() {
  ...
  // FIN ENTORNO DE PRUEBAS
    ...
    let codIdioma = languageSelectedList[1];
    const goBack = () => {
        window.history.back();
    };
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5053/product/code/${codigoParaPasar}`)
        ...
        .then((data) => {
            ...
            let caca = ingredientList.join(", ");
            setIngredientString(caca);
    ...
    <>
      <div className="documentTranslatedMainDiv">
        ...
        <div className="documentTranslatedSemiFinal">
            ...
            <div className="documentTranslatedFinal" onClick={goBack}><p className="documentTranslatedMainDiv--nextDoc"><DocumentTranslator codIdioma={codIdioma} texto1={"Ir a informe anterior"}/></p>
            </div>
        </div>
    </>
  );
